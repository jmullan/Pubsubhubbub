...
class Subscriber
{
    ...
    public function __construct($hub_url, $callback_url, $credentials = false)
    {
        ...
        if (!preg_match("|^https?://|i", $callback_url)) {
            throw new Exception('The specified callback url does not appear to be valid: '.$callback_url);
        }
    ...
    // $use_regexp lets you choose whether to use google AJAX feed api (faster,
    // but cached) or a regexp to read from site
    public function findFeed($url, $http_function = false)
    {
    ...
    private function changeSubscription($mode, $topic_url, $http_function = false)
    {
        if (!isset($topic_url)) {
